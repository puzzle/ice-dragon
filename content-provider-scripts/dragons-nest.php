...
// ----------------------------- COPY FROM https://github.com/adhocore/php-jwt/blob/master/src/JWTException.php -------------------------------
...
// ----------------------------- COPY FROM https://github.com/adhocore/php-jwt/blob/master/src/ValidatesJWT.php -------------------------------
...
// ----------------------------- COPY FROM https://github.com/adhocore/php-jwt/blob/master/src/JWT.php -------------------------------
...
function verifyVoucher($voucherJWT) {
    // try to decode JWT token
    $jwt = new JWT($GLOBALS['paymentConfirmationSecret'], $GLOBALS['hmacAlgorithm']);
    $payload = $jwt->decode($voucherJWT);

    // if no exception was thrown, voucher was valid
    return $payload;
}

function hasIceDragonCookie() {
    if(isset($_COOKIE['ICE_DRAGON'])){
        if(verifyVoucher($_COOKIE['ICE_DRAGON'], $GLOBALS['paymentConfirmationSecret'], $GLOBALS['hmacAlgorithm'])) {
            return true;
        }
    }
    return false;
